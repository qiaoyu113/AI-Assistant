<template>
  <div :id="'app'">
    ...
    <van-overlay :show="$store.state.overlay" style="opacity: 0; background-color: rgba(0, 0, 0, 0);" />
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...mapState({
      cachedViews: (state) => { return state['cached-views'].cachedViews }
    }),
    transitionName() {
  ...
  watch: {
    $route(to, from) {
      ...
      // if (from.meta && to.meta && from.meta.keepAlive) {
...
#app {
  ...
  position: relative;
  .van-nav-bar{
    background: url('../src/icons/title.png') no-repeat 0 0 !important;
